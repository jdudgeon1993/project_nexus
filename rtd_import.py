...
Downloads RTD's GTFS feed and imports N, B, G line data to Supabase
...
# Try to get from environment variables first (for GitHub Actions)
# Fall back to hardcoded values for local testing
SUPABASE_URL = os.getenv("SUPABASE_URL", "https://exojuwforrrtewccqjfu.supabase.co")
SUPABASE_KEY = os.getenv("SUPABASE_KEY", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImV4b2p1d2ZvcnJydGV3Y2NxamZ1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjY5NjYwNzcsImV4cCI6MjA4MjU0MjA3N30.ZE-vLmDg9y4FxLby3AEOGYyJcYLk0Tvazwl94CdzjUI")
...
# Based on the Supabase table schemas
FALLBACK_SCHEMA = {
    'rtd_routes': [
...
def main():
    ...
    try:
        ...
        supabase.table('rtd_stop_times').delete().neq('id', 0).execute()
        supabase.table('rtd_trips').delete().neq('trip_id', '').execute()
        supabase.table('rtd_calendar_dates').delete().neq('id', 0).execute()
        supabase.table('rtd_calendar').delete().neq('service_id', '').execute()
        supabase.table('rtd_stops').delete().neq('stop_id', '').execute()
        supabase.table('rtd_routes').delete().neq('route_id', '').execute()
